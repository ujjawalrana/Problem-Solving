...
ll help(vector<ll>&a,ll start,ll end,ll ok){
    ...
    if(ok){
        // Player 1 maximizing his score
        return dp[start][end]=max(a[start]-help(a,start+1,end,0),a[end]-help(a,start,end-1,0));
    }
    else{
        // Player 2 maximizing his score
        return dp[start][end]=max(a[start]-help(a,start+1,end,1),a[end]-help(a,start,end-1,1));
    }
}
